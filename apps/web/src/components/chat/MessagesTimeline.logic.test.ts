...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("keeps a running call visible when a parallel call completes", () => {
    const rows = deriveMessagesTimelineRows({
      timelineEntries: [
        {
    ...
    expect(rows.find((row) => row.kind === "work-live")).toMatchObject({
      entry: { id: "work-running", toolLifecycleStatus: "inProgress" },
      groupedEntries: [{ id: "work-running" }, { id: "work-completed" }],
    });
  });

  it("returns a completed trailing call to history while the turn keeps thinking", () => {
    const rows = deriveMessagesTimelineRows({
      timelineEntries: [
        {
          id: "work-completed-entry",
