...
export function deriveMessagesTimelineRows(input: {
  ...
  const workEntryCouldBeInProgress = (entry: WorkLogEntry) =>
    ...
    activeEntries.some((entry) => {
      ...
      if (entry.kind === "work") {
        return (
          ...
          workEntryCouldBeInProgress(entry.entry)
        );
      }
  ...
  for (let index = 0; index < input.timelineEntries.length; index += 1) {
    ...
    while (cursor < input.timelineEntries.length) {
      ...
      if (isVisibleActiveToolEntry(nextEntry.entry)) {
        if (
          nextEntry.entry.toolLifecycleStatus === "inProgress" ||
          latestToolEntry.entry.toolLifecycleStatus !== "inProgress"
        ) {
    ...
    if (
      hasLaterTurnContent[cursor] !== true &&
      visibleBatchEntries.some(workEntryCouldBeInProgress)
    ) {
